...
const NewOrderPage = () => {
  ...
  const handlePrepare = () => {
    ...
    setOrders(
      orders.map((o) =>
        o.id === id
          ? { ...o, status: "preparing" as const, trackingNumber: tracking }
          : o
      )
    );
    setNotifications([
  ...
  const handleCancel = () => {
    // Check if seller has enough balance to refund
    ...
    // Record the transaction as a deduction for the seller
    setTransactions([
      {
        id: `t${Date.now()}`,
        type: "transfer", // Using transfer type for deduction
    ...
    setNotifications([
      {
        id: `n${Date.now()}`,
        type: "order_cancelled_seller" as const,
        title: "Order Cancelled",
        message: `Order for ${
          order.listing.name
        } has been cancelled. Refund of RM ${order.totalPrice.toFixed(
          2
    ...
    toast.info("Order cancelled. Refund processed and deducted from wallet.");
          ...
          <div className="space-y-4">
            <div className="bg-card rounded-xl p-4 border border-border">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => navigate(`/print-label/${trackingNum}`)}
                  className="flex-1 py-2 rounded-lg border border-border text-foreground text-xs font-medium flex items-center justify-center gap-1.5"
                >
            ...
            <div className="bg-accent/30 rounded-xl p-4">
              ...
              <p className="text-sm font-semibold text-foreground text-center">
                Courier will come to pick up within 2 days
              </p>
